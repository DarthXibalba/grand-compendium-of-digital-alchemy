...
- Every `sₕ` for `1 <= h <= k` is in `wordList`. Note that `beginWord` does not need to be in `wordList`.
- `sₖ == endWord`
...
- `wordList[h].length == beginWord.length`
- `beginWord`, `endWord`, and `wordList[h]` consist of lowercase English letters.
- `beginWord != endWord`
